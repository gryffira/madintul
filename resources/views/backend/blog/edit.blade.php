	...
	<div class="page-header page-header-default">
		<div class="page-header-content">
			<div class="page-title">
				<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Edit Berita</span></h4>
			</div>
	...
	<div class="content">
		<div class="row">
			...
			<div class="col-12 col-md-8">
				<!-- Basic layout-->
				<form action="{{ route('blog.update', $post->id) }}" method="post" enctype="multipart/form-data" id='post-form'>
					@csrf
					@method('PATCH')
					<div class="panel panel-flat">
						...
						<div class="panel-body ">
							...
							<div class="form-group {{ $errors->has('body') ? 'has-error' : '' }}">
								<label>Isi</label>
								<textarea rows="5" cols="5" class="form-control" name="body" value="{{$post->body}}">{{$post->body}}</textarea>
								@if($errors->has('body'))
						...
						<div class="category-content">
							<div class="form-group {{ $errors->has('category_id') ? 'has-error' : '' }}">
								...
								<select class="form-control" name="category_id">
									...
									<option value="2" {{ $post->category_id == 2 ? 'selected' : '' }}>Education</option>
									<option value="3" {{ $post->category_id == 3 ? 'selected' : '' }}>Sports</option>
									<option value="4" {{ $post->category_id == 4 ? 'selected' : '' }}>Olympic</option>
									<option value="5" {{ $post->category_id == 5 ? 'selected' : '' }}>Healty</option>
								</select>
					...
					<div class="panel panel-flat" style="height: 200px">
						...
						<div class="category-content">
							<div class="form-group {{ $errors->has('published_at') ? 'has-error' : '' }}">
								<input class="form-control" type="date" name="published_at"  value="{{$post->published_at}}">
